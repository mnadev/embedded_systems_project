...
void BTN_Intr_Handler(void *InstancePtr)
{
	...
	xil_printf("Temperature set to: %d Fahrenheit", temp_set);
...
void RunThermostat() {
   ...
   while (1) {
	...
	write_string("Hello World", "From", "Outer space");
	...
	n = SysUart_Recv(&bt_uart, buf, 1);

	if (n != 0) {
		SysUart_Send(&bt_uart, buf, 1);
		BT2_SendData(&bt2, buf, 1);
	}
